...
# 配置文件
path = '/data/jenkins/deploy/config'
...
# 判断执行结果是否正确
def run_server_job(jenkins_server,jobname,imagetag):
    job_status = jenkins_server[jobname]
    param = {
    ...
    if job_status.is_queued_or_running():
        ...
        jenkins_server.build_job(jobname,params=param)
...
# 运行jenkins job: test
run_server_job(jenkins_server,sys.argv[1],sys.argv[2])
